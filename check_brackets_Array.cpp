    ...
    #include <stdexcept>
    #include <chrono>
    #include "DSStack.cpp" // Don't know why, but if I include the .h file it won't build
    
    using namespace std;
    
    void removeComments(string& s) // processes lines for // comments
    {
        int l = s.length();
        for(int i = 0; i < l; ++i)
        {
            if(s.at(i) == '/' && s.at(i+1) == '/') // if there are ever two //'s next to each other
            {
    ...
    int main(int argc, char **argv)
    {
        ...
        {
            ++lineCount;
            removeComments(inputLine); // cleans the input line of comments
